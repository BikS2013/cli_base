...
1. Install NodeJS 18+, then run:
   ```bash
   npm install -g @anthropic-ai/claude-code
   ```
   **Note:** Do NOT use `sudo npm install -g` as this can lead to permission issues and security risks. If you encounter permission errors, see the section on configuring Claude Code for recommended solutions.
...
Claude Code is in beta as a research preview. We’re gathering developer feedback on AI collaboration preferences, which workflows benefit most from AI assistance, and how to improve the agent experience.
...
- **Node not found errors:** If you see `exec: node: not found` when running `claude`, your WSL environment may be using a Windows installation of Node.js. Confirm this with:
  ```bash
  which npm
  which node
  ```
  These should point to Linux paths starting with `/usr/` rather than `/mnt/c/`. To fix this, try installing Node via your Linux distribution’s package manager or via nvm.
...
claude -p "update the README with the latest changes" --allowedTools "Bash(git diff:*)" "Bash(git log:*)"
```
...
| `env`                      | JSON (e.g., `{"FOO": "bar"}`)         | Environment variables that will be applied to every session |
...
Manage project configuration with:
```bash
...
- **Terminal bell notifications:** Enable sound alerts when tasks complete:
  ```bash
...
- **iTerm 2 system notifications:** For iTerm 2 alerts when tasks complete:
...
- **Compact conversations:** Claude uses auto-compact by default when context exceeds 95% capacity.
